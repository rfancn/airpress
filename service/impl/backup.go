...
func (b *backupServiceImpl) BackupWholeSite(ctx context.Context, toBackupItems []string) (*dto.BackupDTO, error) {
	backupFilename := consts.AirPressBackupPrefix + time.Now().Format("2006-01-02-15-04-05") + util.GenUUIDWithOutDash() + ".zip"
	...
	for _, toBackupItem := range toBackupItems {
		toBackupPath := filepath.Clean(filepath.Join(b.Config.AirPress.WorkDir, toBackupItem))
		if !strings.HasPrefix(toBackupPath, b.Config.AirPress.WorkDir) {
			continue
		}
...
func (b *backupServiceImpl) ListFiles(ctx context.Context, path string, backupType service.BackupType) ([]*dto.BackupDTO, error) {
	...
	case service.WholeSite:
		prefix = consts.AirPressBackupPrefix
	case service.JSONData:
		prefix = consts.AirPressDataExportPrefix
	case service.Markdown:
		prefix = consts.AirPressBackupMarkdownPrefix
	}
...
func (b *backupServiceImpl) ExportData(ctx context.Context) (*dto.BackupDTO, error) {
	...
	data["version"] = consts.AirPressVersion
	...
	backupFilename := consts.AirPressDataExportPrefix + time.Now().Format("2006-01-02-15-04-05") + util.GenUUIDWithOutDash() + ".json"
...
func (b *backupServiceImpl) ListToBackupItems(ctx context.Context) ([]string, error) {
	dirEntrys, err := os.ReadDir(b.Config.AirPress.WorkDir)
	if err != nil {
