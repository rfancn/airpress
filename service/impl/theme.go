...
import (
	...
	"github.com/rfancn/airpress/service/theme"
...
func (t *themeServiceImpl) ListAllTheme(ctx context.Context) ([]*dto.ThemeProperty, error) {
	themes, err := t.PropertyScanner.ListAll(ctx, t.Config.AirPress.ThemeDir)
	if err != nil {
		return nil, err
	}
	activatedThemeID, err := t.OptionService.GetActivatedThemeID(ctx)
...
func (t *themeServiceImpl) addTheme(ctx context.Context, themeProperty *dto.ThemeProperty) (*dto.ThemeProperty, error) {
	...
	err = util.CopyDir(themeProperty.ThemePath, filepath.Join(t.Config.AirPress.ThemeDir, themeProperty.FolderName))
	if err != nil {
		return nil, xerr.WithStatus(err, xerr.StatusBadRequest)
	}
	return t.PropertyScanner.ReadThemeProperty(ctx, filepath.Join(t.Config.AirPress.ThemeDir, themeProperty.FolderName))
}
